...
void AMaze_Player::MoveForward(float Value)
{
	...
	FVector End = Start + CameraComponent->GetForwardVector() * Value * 6;
	...
	if (!Hit.bBlockingHit)
	{
		SetActorLocation(GetActorLocation() + CameraComponent->GetForwardVector() * Value * 6, false, nullptr, ETeleportType::ResetPhysics);
	}
}
